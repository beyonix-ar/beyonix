-- force_delete_product_variant_super_admin / force_delete_product_super_admin
-- se escribieron antes de que existiera inventory_stock_adjustments
-- (20260820180000_manual_stock_adjustment.sql). Como esa tabla no se
-- desacoplaba, guard_product_variant_delete la seguía viendo como
-- "movimientos históricos" y el borrado forzado fallaba igual que el normal.
-- Se agrega su borrado junto al de inventory_variant_allocations /
-- stock_reservations: es estado de trabajo (ajustes manuales), no un
-- documento comercial con snapshot propio, así que se borra en vez de
-- desacoplar (igual criterio que esas dos tablas).
