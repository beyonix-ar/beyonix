-- Formaliza como migración la gestión comercial de devoluciones/notas de
-- crédito (recepción, destino de stock, envíos y liquidación), que hasta
-- ahora solo existía aplicada a mano vía
-- supabase/sql/092_credit_note_return_management.sql (nunca representada en
-- supabase/migrations/). Depende de public.order_credit_notes y
-- public.order_credit_note_items, formalizadas en
-- 20260729181538_formalize_partial_credit_notes.sql. DDL trasladado sin
-- cambios funcionales; todas las sentencias son idempotentes, por lo que
-- reaplicar esto sobre el remoto ya migrado no tiene efecto. Verificado
-- columna por columna, constraint por constraint e índice por índice contra
-- el esquema remoto real (introspección read-only, 2026-08-18).
--
-- Fecha del archivo: igual que la 20260729181538 vecina, 092 también se
-- aplicó a mano el 2026-07-27 (antes del baseline del 2026-07-29), así que
-- va inmediatamente después de esa formalización y no en la fecha en que se
-- escribió este archivo. Mismo motivo: 20260816140000_reset_commercial_test_data.sql
-- y los dos diagnósticos temporales de esa fecha ya operan sobre estas
-- tablas y necesitan que existan antes.
--
-- Única diferencia respecto del 092 original: el UPDATE de backfill de
-- settlement_status/management_status agrega un filtro adicional (ver
-- comentario debajo) para que no sea destructivo si se reaplica sobre datos
-- reales posteriores a la migración manual original.
...
--
-- A diferencia del 092 original, esta versión agrega el filtro por
-- reason = 'Nota de crédito histórica migrada' (el marcador que usa
-- exclusivamente el backfill de 20260818100000_formalize_partial_credit_notes.sql).
-- Sobre el remoto ya migrado esto es un no-op: en el momento en que 092 corrió
-- a mano, todas las notas autorizadas existentes eran justamente esas
-- históricas, así que el filtro no cambia qué filas se tocaron entonces. Pero
-- evita que, si esta migración se reaplica más adelante, pise el
-- settlement_status/management_status de notas reales creadas después por el
-- flujo de gestión granular (que también pueden llegar a status=authorized +
-- destination=customer_balance sin haber pasado todavía por la liquidación).
...
where note.status = 'authorized'
  and note.reason = 'Nota de crédito histórica migrada'
  and (
