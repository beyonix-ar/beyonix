-- Causa raíz del bloqueo "tiene unidades vinculadas... Reasigná primero el
-- stock" al forzar la eliminación de un producto/variante con ventas de
-- Mercado Libre: force_delete_product_super_admin y
-- force_delete_product_variant_super_admin NUNCA desacoplaban
-- mercadolibre_sales. Además, product_cost_entries, business_expenses,
-- inventory_return_movements, external_sales y mercadolibre_sales tienen
-- triggers AFTER INSERT/UPDATE/DELETE (refresh_inventory_after_*) que
-- recalculan productos.stock/producto_variantes.stock apenas se escribe
-- cualquiera de esas tablas. Al desacoplar compras y devoluciones (fuentes
-- positivas) sin tocar la venta de ML (fuente negativa), esos recálculos
-- intermedios dejaban únicamente la venta de ML restando stock, así que la
-- variante terminaba con stock derivado negativo: guard_product_variant_delete
-- veía `stock <> 0` y bloqueaba la eliminación con un mensaje que además era
-- engañoso (no había nada que "reasignar").
--
-- Corrección: se desacopla también mercadolibre_sales (antes no se tocaba en
-- absoluto), y todos los recálculos automáticos intermedios se desactivan
-- mientras se desacoplan las fuentes, para no depender del orden en que cada
-- trigger AFTER dispare sobre filas parcialmente desacopladas. El recálculo
-- final es uno solo, explícito, después de desacoplar todo. Se preserva el
-- mismo modelo ya implementado: SET NULL en vez de borrar el documento
-- histórico (la venta de ML sigue existiendo con su product_name/sku propios,
-- sólo pierde la referencia al catálogo eliminado).
...
begin
  ...
  -- Se desactivan los recálculos automáticos y guardias de las tablas que se
  -- van a desacoplar: el recálculo del stock derivado se hace una sola vez,
  -- explícitamente, más abajo, con todo ya desacoplado.
