...
test("componentes reales: alertas/facturación error ≠ vacío, retry y borrado con confirmación tipada", async () => {
  ...
  try {
    ...
    let updated = 0
    await render(<OrderReplacementManager pedido={{ id: 123, usuario_id: null, estado: "entregado", total: 50000, created_at: "2026-09-19", orden_items: [{ id: 7, orden_id: 123, producto_id: 1, cantidad: 3, precio: 1000, return_restocked_quantity: 2 }] }} onUpdated={async () => { updated++ }} />)
    ...
    const { ReturnInventoryPanel } = await import("../../components/claims/admin-claim-manager")
    ...
    assert.match(document.body.textContent || "", /Vendió 5 · Cliente reclama 3 · Recibimos 2 · Queda recibir 1/)
    ...
    await act(async () => [...document.querySelectorAll("button")].find((button) => button.textContent === "Guardar recepción")!.click())
    ...
    assert.match(document.body.textContent || "", /Vendió 5 · Cliente reclama 3 · Recibimos 3 · Queda recibir 0/)
