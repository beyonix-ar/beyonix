...
test('Auditoría 7: permisos y carreras con backends PostgreSQL independientes', { timeout: 120000 }, async (t) => {
  ...
  try {
    ...
    // embedded-postgres.stop() espera 'exit', que en Windows puede llegar
    // antes de cerrar los pipes del hijo. Esperar 'close' libera esos handles
    // antes de que el runner intente terminar.
    const postgresClosed = server.process ? once(server.process, 'close') : null
    await server.stop()
    if (postgresClosed) await postgresClosed
  }
})
