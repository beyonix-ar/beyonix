...
export function InsufficientStockModal({
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 px-4 py-5 backdrop-blur-sm">
      ...
      <div className="relative z-10 w-[min(400px,calc(100vw-32px))] rounded-2xl border border-red-400/24 bg-[#080D13] p-4 shadow-[0_28px_90px_rgba(0,0,0,0.72)]">
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="flex-1 text-[15px] font-bold leading-tight text-white">
            {INSUFFICIENT_STOCK_TITLE}
          </h2>
          ...
          <button
            ...
            aria-label="Cerrar"
            onClick={onClose}
            className="flex size-7 shrink-0 cursor-pointer items-center justify-center rounded-full text-white/50 transition-colors hover:bg-white/8 hover:text-white"
          >
            <X className="size-3.5" />
          </button>
        </div>

        <p className="mt-2.5 text-[13px] leading-5 text-white/65">
          {isPlural
            ? INSUFFICIENT_STOCK_MESSAGE_PLURAL
            : INSUFFICIENT_STOCK_MESSAGE_SINGULAR}
        </p>

        <p className="mb-1.5 mt-3.5 text-[10px] font-bold uppercase tracking-widest text-white/40">
          {isPlural ? "Productos afectados" : "Producto afectado"}
        </p>

        <ul className="divide-y divide-white/[0.06] rounded-lg border border-white/8 bg-white/[0.03] px-3">
          {items.map((item) => (
            <li
            ...
            >
              ...
              {item.variantName && (
                <p className="mt-0.5 flex items-center gap-1.5 text-[11.5px] text-white/50">
